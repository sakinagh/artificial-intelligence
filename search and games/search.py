...
def iterativeDeepeningSearch(problem):
    ...
    limit = 0
    count_loop = 0
    limit_loop = 999999999

    while True:
        if count_loop == limit_loop:
            break
        ...
        while not frontier.isEmpty():
            ...
            if depth < limit:
                for action in problem.getActions(state):
                    child_state = problem.getResult(state, action)
                    
                    if child_state not in explored and child_state not in [node[0] for node in frontier.list]:
                        frontier.push((child_state, actions + [action], depth + 1))
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not frontier.isEmpty():
        ...
        for action in problem.getActions(state):
            child_state = problem.getResult(state, action)
            child_path_cost = path_cost + problem.getCost(state, action)
            frontier.push((child_state, actions + [action], child_path_cost), child_path_cost + heuristic(child_state, problem))
