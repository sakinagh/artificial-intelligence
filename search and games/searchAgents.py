...
class CornersProblem(search.SearchProblem):
    ...
    def goalTest(self, state):
        ...
        if position in self.corners:
            corner_index = self.corners.index(position)
            visited_corners = list(visited_corners)
            visited_corners[corner_index] = True
            return tuple(visited_corners) == (True, True, True, True)
        return False
        #util.raiseNotDefined()
    ...
    def getResult(self, state, action):
        ...
        return (new_position, tuple(new_visited))
...
def cornersHeuristic(state, problem):
    ...
    for corner in unvisited_corners:
        if walls[position[0]][position[1]] or walls[corner[0]][corner[1]]:
            min_distance = min(min_distance, mazeDistance(position, corner, problem.startingGameState))
